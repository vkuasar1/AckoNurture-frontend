...
const typeConfig = {
  weight: {
    ...
    emptySubtext:
      "Track your little one's weight and watch them grow stronger every day",
  ...
  head: {
    ...
    emptySubtext:
      "Head circumference is a key indicator of healthy brain development",
...
export default function BabyCareGrowth() {
  ...
  const growthDiff =
    latestEntry && previousEntry
      ? (
          parseFloat(latestEntry.value) - parseFloat(previousEntry.value)
        ).toFixed(1)
      : null;
  ...
  const randomQuote =
    inspirationalQuotes[
      Math.floor(Date.now() / 86400000) % inspirationalQuotes.length
    ];
  ...
  return (
    <div className="app-container bg-zinc-50 min-h-screen flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pb-24 px-4 pt-4">
        ...
        {latestEntry && (
          <Card
            className={`${config.bgLight} border ${config.borderColor} rounded-2xl mb-4 overflow-hidden`}
            data-testid="card-growth-insight"
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[12px] text-zinc-500">
                    Last updated{" "}
                    {format(new Date(latestEntry.recordedAt), "MMM d, yyyy")}
                  </p>
                </div>
        ...
        {/* Fun Tip Card */}
        <Card
          className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100 rounded-2xl mb-4"
          data-testid="card-fun-tip"
        >
        ...
        {filteredEntries.length > 1 && (
          <Card
            className="bg-white border border-zinc-100 rounded-2xl mb-4 overflow-hidden"
            ...
            <CardContent className="p-4">
              ...
              <div className="flex items-end gap-2 h-20">
                {filteredEntries
                  ...
                  .reverse()
                  .map((entry, idx) => {
                    const values = filteredEntries.map((e) =>
                      parseFloat(e.value),
                    );
                    ...
                    const heightPx =
                      ((parseFloat(entry.value) - min) / range) * 56 + 16;

                    return (
                      <div
                        key={entry.id}
                        className="flex-1 flex flex-col items-center gap-1"
                        data-testid={`chart-bar-${entry.id}`}
                      >
                        <div
        ...
        {isLoading ? (
            ...
            {filteredEntries.map((entry, idx) => (
              ...
              >
                <CardContent className="p-3 flex items-center justify-between">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p
                          ...
                          data-testid={`entry-value-${entry.id}`}
                        >
                  ...
                  {entry.percentile && (
                    <span
                      className={`text-[12px] font-semibold ${config.textColor} ${config.bgLight} px-2.5 py-1 rounded-full`}
                      data-testid={`entry-percentile-${entry.id}`}
                    >
          ...
          /* Rich Empty State */
          <Card
            className={`bg-gradient-to-br ${config.bgLight} border-2 border-dashed ${config.borderColor} rounded-2xl overflow-hidden`}
            ...
            <CardContent className="p-8 text-center">
              ...
              <div className="flex items-center justify-center gap-2 mb-4">
                ...
                <span className="text-[12px] text-zinc-400">
                  Keep track of every precious moment
                </span>
              </div>
              <Button
