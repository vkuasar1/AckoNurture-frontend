...
export default function BabyCareWelcome() {
  ...
  const initialStep = params.get("step") === "selection" ? "selection" : "welcome";
  
  // Onboarding step state
  const [onboardingStep, setOnboardingStep] = useState<OnboardingStep>(initialStep);
  const [selectedOptions, setSelectedOptions] = useState<Set<OnboardingSelection>>(new Set());
  
  const { data: profiles = [], isLoading: isLoadingProfiles } = useQuery<BabyProfile[]>({
  ...
  useEffect(() => {
    ...
    // and the user has existing profiles
    if (!isLoadingProfiles && profiles.length > 0 && initialStep !== "selection") {
      setLocation(`/babycare/home/${profiles[0].id}`);
    }
  }, [profiles, isLoadingProfiles, initialStep, setLocation]);
  ...
  const createDemoProfile = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiRequest("POST", "/api/baby-profiles", demoProfile);
      return res.json();
    },
    onSuccess: (newProfile: BabyProfile) => {
      queryClient.invalidateQueries({ queryKey: ["/api/baby-profiles"] });
      setLocation(`/babycare/home/${newProfile.id}`);
    },
    onError: () => {
      ...
      <div className="app-container min-h-screen bg-zinc-50 flex flex-col">
        ...
        <div className="bg-[#1a1a1a] text-white px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[13px] font-medium bg-gradient-to-r from-pink-400 to-violet-400 bg-clip-text text-transparent">Nurture</span>
            <div className="w-9" />
          </div>
        </div>
        ...
        <div className="flex-1 px-6 pt-8 pb-6 flex flex-col">
          ...
          >
            <h1 className="text-[28px] font-bold text-zinc-900 leading-tight mb-2">
              What would you like<br />to track?
            </h1>
            <p className="text-[15px] text-zinc-500 mb-8">
              Whether you're a parent, grandparent, or caregiver — we've got you covered.
            </p>
          </motion.div>
          ...
          <div className="space-y-4 flex-1">
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${
                  selectedOptions.has("baby") ? "bg-white/20" : "bg-violet-100"
                }`}>
                  <Baby className={`w-8 h-8 ${selectedOptions.has("baby") ? "text-white" : "text-violet-600"}`} />
                </div>
                <div className="flex-1">
                  <h3 className={`text-[18px] font-bold mb-1 ${selectedOptions.has("baby") ? "text-white" : "text-zinc-900"}`}>
                    Baby Care
                  </h3>
                  <p className={`text-[13px] leading-snug ${selectedOptions.has("baby") ? "text-white/80" : "text-zinc-500"}`}>
                    Track vaccines, growth, milestones & get AI guidance for your child
                  </p>
                </div>
                {selectedOptions.has("baby") && (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${
                  selectedOptions.has("mother") ? "bg-white/20" : "bg-pink-100"
                }`}>
                  <Heart className={`w-8 h-8 ${selectedOptions.has("mother") ? "text-white" : "text-pink-600"}`} />
                </div>
                <div className="flex-1">
                  <h3 className={`text-[18px] font-bold mb-1 ${selectedOptions.has("mother") ? "text-white" : "text-zinc-900"}`}>
                    My Wellness
                  </h3>
                  <p className={`text-[13px] leading-snug ${selectedOptions.has("mother") ? "text-white/80" : "text-zinc-500"}`}>
    ...
    <div className="app-container min-h-screen bg-white flex flex-col">
      ...
      <div className="relative overflow-hidden bg-gradient-to-b from-violet-50 via-pink-50/50 to-white">
        ...
        <div className="relative flex flex-col items-center pt-10 pb-14 px-6">
          ...
          >
            Welcome to <span className="bg-gradient-to-r from-pink-500 to-violet-500 bg-clip-text text-transparent">Nurture</span>
          </motion.h1>
      ...
      <Dialog open={showHospitalDialog} onOpenChange={setShowHospitalDialog}>
        <DialogContent className="max-w-[340px] rounded-2xl" data-testid="dialog-hospital-options">
          <DialogHeader>
            ...
            <DialogDescription className="text-[14px] text-zinc-500 text-center mt-2">
              If you were registered at a partner hospital, you can quickly set up your baby's profile.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className="text-[12px] text-zinc-500">Use camera to scan</p>
                </div>
              </Button>
      ...
      {/* Code Entry Dialog */}
      <Dialog open={showCodeEntry} onOpenChange={(open) => { setShowCodeEntry(open); setCodeError(""); setRegistrationCode(""); }}>
        <DialogContent className="max-w-[340px] rounded-2xl" data-testid="dialog-code-entry">
          <DialogHeader>
            ...
            <DialogDescription className="text-[14px] text-zinc-500 text-center mt-2">
              Enter the registration code provided by your hospital during admission.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="regCode" className="text-[13px] font-medium">Registration Code</Label>
                <Input
                  id="regCode"
                  placeholder="e.g., APOLLO2024"
                  value={registrationCode}
                  onChange={(e) => { setRegistrationCode(e.target.value); setCodeError(""); }}
                ...
                />
                {codeError && (
                  <p className="text-[12px] text-red-500 mt-2 text-center" data-testid="text-code-error">
                    {codeError}
                  </p>
                )}
              </div>
      ...
      {/* QR Scanner Dialog */}
      <Dialog open={showScanner} onOpenChange={(open) => { setShowScanner(open); setScanStatus("idle"); }}>
        <DialogContent className="max-w-[340px] rounded-2xl" data-testid="dialog-qr-scanner">
          <DialogHeader>
            <DialogTitle className="text-[18px] font-bold text-center">Scan Hospital QR Code</DialogTitle>
            <DialogDescription className="sr-only">
              Use your camera to scan the QR code from your hospital registration
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                {scanStatus === "found" && (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-white text-[14px] font-medium">Code Found!</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="text-center">
              ...
              {scanStatus === "found" && (
                <p className="text-[14px] text-emerald-600 font-medium" data-testid="text-scan-success">
            ...
            <Button
              variant="outline"
              onClick={() => { setShowScanner(false); setShowCodeEntry(true); setScanStatus("idle"); }}
